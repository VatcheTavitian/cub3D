...
/*   Updated: 2023/10/18 14:14:02 by vtavitia         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	info->hands_img = \
	initialise_text_images_helper(info, "./textures/hands.xpm");
	info->gun_wall_img = \
	initialise_text_images_helper(info, "./textures/gun_wall.xpm");
}

void	destroy_images(t_info *info)
{
	mlx_destroy_image(info->mlx_ptr, info->n_img->img);
	mlx_destroy_image(info->mlx_ptr, info->s_img->img);
	mlx_destroy_image(info->mlx_ptr, info->e_img->img);
	mlx_destroy_image(info->mlx_ptr, info->w_img->img);
	free(info->n_img);
	free(info->w_img);
	free(info->s_img);
	free(info->e_img);
